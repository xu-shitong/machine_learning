...
\textbf{SVM}
  ...
  \quad \quad \quad \quad $=\frac{1}{2}\vec{\alpha}^T (\textbf{x} * y)^T(\textbf{x} * y) \vec{\alpha} - \sum_{i=1}^{|B|}\vec{\alpha}^{(i)}$
  ...
  \quad \quad \textbf{常见错误}：

  \quad \quad \quad 1.当样本有重叠，无法使用硬性分类，通过拉格朗日乘数法得到的权重和偏差无法有效分类数据。
