...
\textbf{Probability Density Estimate PDE}
  ...
  \quad \quad \quad \quad $\mu = \frac{1}{N}\sum x^{(i)}$

  \quad \quad \quad \quad $\sigma = \frac{1}{|S|}\sum(x^{(i)} - \mu)^2$
  ...
  \quad \quad \quad 当$\frac{d\mathcal{L}}{d\mu} = 0$和$\frac{d\mathcal{L}}{d\mu} = 0$，$\mu$和$\sigma$有以上公式求解\\
\textbf{Gaussian Mixtures}：一种PDE

  使用多个子分部之和代表样本pdf分部，$p(x) = \sum_{k=1}^{K} \pi_kp_k(x)$

  \quad $0 \leq \pi_k \leq 1$, $\sum_{k=1}^{K}\pi_k = 1$，保证产生的pdf 积分为1

  \textbf{GM Model(GMM)}：假设所有子分部都为正态分部

  \quad 参数：$\theta = \{ \pi_k, \mu_k, \varSigma_k | k = 1..K\}$
  ...
  \quad 4.当$\theta$不再大幅改变则停止，否则回到2.

  调参：选择子分部个数K

  \quad $BIC_K = \mathcal{L}(K) + \frac{P_K}{2}log(|B|)$

  \quad \quad $\mathcal{L}(K)$为K类别的neg. log likelihood
