...
\textbf{confusion matrix困惑矩阵}：分析二元/多元分类
  ...
  \quad 即$P($计算结果匹配 $|$ 计算结果为正$)$：所有计算为真的样本中预测正确的概率

  \textbf{recall} = $\frac{TP}{TP + FN}$

  \quad 即$P($计算结果匹配 $|$ 期望结果为正$)$：所有期望为真的样本被正确预测的概率
  ...
  \textbf{$F_\beta$} $= (1 - \beta^2) \cdot \frac{precision \cdot recall}{(\beta^2 \cdot precision) + recall}$

  \quad $\beta$：当precision为recall$\beta$倍重要
